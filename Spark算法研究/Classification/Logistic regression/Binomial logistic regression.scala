...
与多分类回归的不同：
    ：不需设置.setFamily("multinomial")
    ：可以使用 trainingSummary.asInstanceOf[BinaryLogisticRegressionTrainingSummary] 评估模型指标（F-Measure 、ROC、PR）
...
lr Parameter setters：
    ...
    lr
      .setElasticNetParam(0.8)   // lr.elasticNetParam.doc ->(the ElasticNet mixing parameter, in range [0, 1]. For alpha = 0, the penalty is an L2 penalty. For alpha = 1, it is an L1 penalty
        ratio:在0到1之间，代表在l1惩罚和l2惩罚之间，如果l1_ratio=1，则为lasso，是调节模型性能的一个重要指标。
